...
public static class Endpoints
{
    ...
    private static async Task<IResult> Delete(string resource,Guid id,HttpContext c,FinoraDbContext db) { var uid=FinanceService.UserId(c.User); object? entity=resource switch {"accounts"=>await db.Accounts.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"categories"=>await db.Categories.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"transactions"=>await db.Transactions.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"budgets"=>await db.Budgets.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"mortgages"=>await db.Mortgages.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"household-bills"=>await db.HouseholdBills.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"medical-expenses"=>await db.MedicalExpenses.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"supermarket-expenses"=>await db.SupermarketExpenses.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),"recurring-transactions"=>await db.RecurringTransactions.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==uid),_=>null}; if(entity is null)return Results.NotFound(); db.Remove(entity); await db.SaveChangesAsync(); return Results.NoContent(); }
}
