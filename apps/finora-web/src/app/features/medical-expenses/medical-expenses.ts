...
@Component({
  standalone: true,
  imports: [CurrencyPipe, DatePipe, ReactiveFormsModule, MatButtonModule, MatCardModule, MatCheckboxModule, MatFormFieldModule, MatIconModule, MatInputModule, MatSelectModule, ExportMenuComponent],
  template: `
    <header><p class="eyebrow">HEALTH & WELLBEING</p><div class="header-row"><div><h1>Medical expenses</h1><p>Keep pharmacy purchases, appointments, exams, and other health costs organised.</p></div><app-export-menu [data]="exportData()" /></div></header>
...
export class MedicalExpensesComponent implements OnInit {
  ...
  exportData():ExportData{const expenses=this.expenses();return{title:'Finora Medical Expenses',fileName:'finora-medical-expenses',columns:['Description','Type','Provider','Amount','Currency','Date','Reimbursable','Notes'],rows:expenses.map(item=>[item.description,this.typeLabel(item.expenseType),item.provider,item.amount.toFixed(2),item.currencyCode,item.expenseDate,item.isReimbursable?'Yes':'No',item.notes]),summary:[`Expenses: ${expenses.length}`,`Current month total: ${this.currentMonthTotal().toFixed(2)} EUR`]};}
