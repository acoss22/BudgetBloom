...
@Component({
  ...
  template: `
    ...
    <div class="layout">
      <mat-card class="form-card"><h2>Register a mortgage</h2>
        <form [formGroup]="form" (ngSubmit)="save()">
          ...
          <button mat-flat-button type="submit" [disabled]="form.invalid || saving() || dateError() || balanceError()">{{saving() ? 'Saving…' : 'Register mortgage'}}</button>
        </form>
      </mat-card>
      <section class="loans" aria-live="polite">
        ...
        @for (loan of mortgages(); track loan.id) {
          <mat-card class="loan"><div class="loan-head"><div><h2>{{loan.name}}</h2><p>{{loan.lender}}@if(loan.propertyAddress){ · {{loan.propertyAddress}}}</p></div><button mat-icon-button (click)="remove(loan)" [attr.aria-label]="'Delete ' + loan.name"><mat-icon>delete_outline</mat-icon></button></div>
  ...
  styles: [`header h1{font-size:clamp(1.8rem,4vw,2.6rem);margin:.15rem 0}.eyebrow{letter-spacing:.15em;color:#397454;font-weight:700}.layout{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(320px,.85fr);gap:1.25rem;margin-top:2rem;align-items:start}mat-card{padding:1.3rem}.form-card h2{margin-bottom:1.3rem}.fields{display:grid;grid-template-columns:1fr 1fr;gap:0 1rem}.wide{grid-column:1/-1}form>button{width:100%;margin-top:.5rem}.loans{display:grid;gap:1rem}.empty{min-height:220px;display:grid;place-content:center;text-align:center;color:#66756d}.empty mat-icon{margin:auto;font-size:42px;width:42px;height:42px;color:#397454}.loan-head{display:flex;justify-content:space-between;gap:1rem}.loan h2{margin-bottom:.25rem}.loan p,.caption,dt{color:#68766f}.loan>strong{display:block;font-size:1.8rem;margin-top:1.2rem}.caption{font-size:.85rem}.progress{height:8px;background:#e2e9e4;border-radius:10px;overflow:hidden;margin:1rem 0}.progress span{display:block;height:100%;background:#5c9b70}dl{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin:0}dt{font-size:.75rem}dd{margin:.2rem 0 0;font-weight:600}.error{color:#b3261e;font-size:.85rem}@media(max-width:1000px){.layout{grid-template-columns:1fr}}@media(max-width:600px){.fields,dl{grid-template-columns:1fr}.wide{grid-column:auto}}`]
})
export class MortgagesComponent implements OnInit {
  private readonly http = inject(HttpClient);
  mortgages = signal<Mortgage[]>([]); loading = signal(true); saving = signal(false);
  form = new FormGroup({
  ...
  save(){if(this.form.invalid||this.dateError()||this.balanceError())return;this.saving.set(true);this.http.post<Mortgage>('/api/mortgages',this.form.getRawValue()).subscribe({next:x=>{this.mortgages.update(items=>[...items,x].sort((a,b)=>a.endDate.localeCompare(b.endDate)));this.form.reset({rateType:1,paymentDay:1,currencyCode:'EUR'});this.saving.set(false)},error:()=>this.saving.set(false)});}
